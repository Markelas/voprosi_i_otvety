...
- **Очередь микротасок (Microtask Queue)** — высокий приоритет. Примеры: колбэки **Promise.then / catch / finally**, **queueMicrotask()**, **MutationObserver**, в Node.js — **process.nextTick** (ещё выше микротасок).
  <u>requestAnimationFrame</u>, так как задач, которые нужно выполнить перед обновлением следующего кадра, может быть несколько, поэтому есть своя отдельная очередь.
...
Зачем нужна цепочка микротасок? Цепочка очереди в промисах, она постоянно ставит новую задачу в микротасках. За счет непрерывности цепочка выполняется, иначе асинхронные события рвали бы цепочки промисов. 
