...
Т.е. при запуске открывается приложение и под капотом, запускается браузер
...
## Подводные камни при разработке PWA

### 1. Сломанный сайт
Если сайт сломается в PWA:
- ❌ Нет кнопки обновления (как в браузере)
- ❌ Пользователь может только выйти из приложения
- ✅ **Решение:** Добавлять кнопки "Повторить" или "На главный экран"


### 2. Нет адресной строки
- ❌ Нельзя скопировать ссылку
- ✅ **Решение:** Добавлять кнопку "Поделиться" в интерфейсе

### 3. Проблемы с поддоменами
- Не рекомендуется разносить PWA по разным поддоменам (`shop.example.com`, `blog.example.com`)
- Документация советует **один домен** для всего приложения
- Иначе проблемы с Service Worker и манифестом

### 4. Push - уведомления, как работают
1. Service Worker создает **подписку**
2. Объект подписки отправляется на сервер
3. Сервер вызывает **сторонний сервис** (Firebase, Apple Push)
4. Сторонний сервис доставляет уведомление на устройство
	❌ Проблема, что это сторонний сервис и не каждая компания захочет отправлять их, например, коды для входа и т.д.


### Почему Web API важны?
Они дают доступ к **нативным возможностям**:
