...
1. **Request Memoization** — дедупликация одинаковых fetch-запросов в одном рендере (Сервер)
2. **Data Cache** — кэш результатов fetch (бессрочно по умолчанию) (Сервер)
3. **Full Route Cache** — кэш отрендеренных страниц (для SSG/ISR) (Сервер)
4. **Router Cache** — кэш страниц на клиенте (в памяти браузера) (Браузер)
