...
**Reconciliation (согласование)** — это процесс, в котором React сравнивает новое дерево компонентов (или Virtual DOM) со старым и решает, какие изменения нужно внести в реальный DOM. Цель: обновить экран с минимальным числом операций. React опирается на эвристики: один и тот же тип элемента на одном уровне — обновляем узел; разный тип — удаляем старый и создаём новый; в списках важны стабильные ключи, чтобы понимать, какой элемент куда переместился. Сравнение делается за один проход по дереву (сверху вниз), без полного перебора всех вариантов. Результат согласования потом применяется к DOM на фазе коммита.
...
Reconciliation относится к фазе **Render (рендер)**.

- **Render-фаза:** React строит новое дерево (или обновлённое представление), сравнивает его со старым и помечает, что нужно сделать с DOM: добавить узел, обновить, удалить. Это «подготовка списка изменений». В архитектуре Fiber эту фазу можно прерывать и возобновлять.
- **Commit-фаза:** React применяет накопленный список изменений к реальному DOM. Выполняется синхронно, без прерываний, чтобы пользователь не видел «полусобранный» интерфейс.

На собеседовании можно сказать: reconciliation — это то, что происходит на фазе рендера; коммит — это уже применение результата к DOM.
...
| На какой фазе происходит reconciliation? | На фазе Render; к DOM изменения применяются на фазе Commit. |
...
Reconciliation — это сравнение нового дерева компонентов со старым и вычисление минимального набора изменений для DOM. React сравнивает по типу элемента и по ключам в списках; один тип — обновляем узел, разный — заменяем целиком. В списках стабильные уникальные ключи нужны, чтобы правильно сопоставлять элементы и не пересоздавать лишние узлы. Сравнение делается на фазе Render, применение к DOM — на фазе Commit. В React 16+ это реализовано в архитектуре Fiber.
