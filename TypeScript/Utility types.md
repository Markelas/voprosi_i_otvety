...
### 🔀 Отличия похожих utility types:

| Пара                | Что делает                    | Работает с         | Пример                                           |
| ------------------- | ----------------------------- | ------------------ | ------------------------------------------------ |
| **`Pick<T, K>`**    | Выбирает свойства из объекта  | Объекты (свойства) | `Pick<User, "id" \| "name">`                     |
| **`Extract<T, U>`** | Извлекает типы из union       | Union типы         | `Extract<"a" \| "b" \| "c", "a" \| "d">` → `"a"` |
|  |  | |  |
|  |  | |  |
| **`Omit<T, K>`**    | Исключает свойства из объекта | Объекты (свойства) | `Omit<User, "password">`                         |
| **`Exclude<T, U>`** | Исключает типы из union       | Union типы         | `Exclude<"a" \| "b" \| "c", "a">` → `"b" \| "c"` |
|  |  | |  |
|  |  | |  |
| **`Parameters<T>`** | Извлекает параметры функции   | Функции            | `Parameters<(a: number) => void>` → `[number]`   |
| **`ReturnType<T>`** | Извлекает возвращаемый тип    | Функции            | `ReturnType<() => string>` → `string`            |

**Ключевое отличие:** `Pick`/`Omit` работают с **свойствами объектов**, а `Extract`/`Exclude` — с **union типами**.
